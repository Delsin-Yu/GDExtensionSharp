...
namespace GDExtensionSharp.NativeInterop;
...
public partial class VariantUtils
{
    ...
    public static godot_variant CreateFrom<[MustBeVariant] T>(in T from)
    {
        ...
        if (typeof(T) == typeof(GDExtensionSharp.Collections.Dictionary))
            return CreateFromDictionary(UnsafeAs<GDExtensionSharp.Collections.Dictionary>(from));

        if (typeof(T) == typeof(GDExtensionSharp.Collections.Array))
            return CreateFromArray(UnsafeAs<GDExtensionSharp.Collections.Array>(from));
